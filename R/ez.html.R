          ...
          if(any(class(Resultats[[i]])=="numeric") ) {
            if(length(Resultats[[i]])==1) {
              ...
              essai<-c("```{r, echo=F, results='asis'}",
                       ...
                       paste0("if(any(grepl('",txt_p_dot_val,"', names(tableau)))) {"),
                       paste0("col<-which(grepl('",txt_p_dot_val,"', names(tableau)))"),
                       "if(length(col)>1) {is<-unique(unlist(apply(tableau[,col], 2,myf )))+1",
                       "tableau[,col]<-apply(tableau[,col], 2, round.ps) }else{",
                       paste0("is<-which(tableau[, which(grepl('",txt_p_dot_val,"', names(tableau)))]<0.05)"),
		       "is<-is+1",
                       paste0("tableau[, which(grepl('",txt_p_dot_val,"', names(tableau)))]<-round.ps(tableau[,  which(grepl('",txt_p_dot_val,"', names(tableau)))])}}"),
