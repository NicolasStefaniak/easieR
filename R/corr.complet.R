corr.complet <-
function(X=NULL, Y=NULL, Z=NULL,data=NULL,  group=NULL, param=c("test paramétrique", "test non paramétrique","Test robustes - impliquant des bootstraps", "Facteurs bayesiens"), 
                       sauvegarde=F, outlier=c("Données complètes", "Identification des outliers","Analyse sans les valeurs influentes"),  z=NULL, info=T, n.boot=NULL, rscale=0.353, html=T){options (warn=-1) 
  ...
  corr.complet.out<-function(X=NULL, Y=NULL, Z=NULL, data=NULL, choix=NULL, group=NULL, param=NULL,n.boot=NULL, rscale=0.353) {
    ...
    if(any(param=="Bayes") | any(param=="Facteurs bayesiens") | any(param=="param") | any(param=="Test paramétrique"))  {
      ...
      Resultats$"Test paramétrique"$Graphiques<-graphiques
    }
    if(any(param=="param") | any(param=="Test paramétrique")){
      ...
      if(choix!="Corrélations") {
        ...
        Resultats$"Test paramétrique"$"Corrélation de Bravais Pearson"<-round(data.frame("r"=BP$estimate,"r.deux"=BP$estimate^2, "IC lim inf"=BP$conf.int[1],"IC lim sup"=BP$conf.int[2], "t"=BP$statistic, 
      ...
      if(!is.null(group)){  
        ...
        if(choix!="Corrélations") Resultats$"Test paramétrique"$"Corrélation partielle de Bravais-Pearson par groupe"<-BPgroup else Resultats$"Corrélation de Bravais-Pearson par groupe"<-BPgroup
    ...
    if(any(param=="non param")| any(param=="Test non paramétrique")){
      ...
      Resultats$"Test non paramétrique"$Graphiques<-graphiques
      ...
      if(choix!="Corrélations") {
        ...
        Resultats$"Test non paramétrique"$"Rho partiel/semi partiel de Spearman"<-spear
        tau<-round(tau,4)
        dimnames(tau)<-list(c("Tau partiel de Kendall","Tau semi-partiel de Kendall"), c("tau", "z", "valeur.p"))
        Resultats$"Test non paramétrique"$"Tau partiel/semi-partiel de Kendall"<-tau
      } else { Spear<-cor.test(data[,X1], data[ ,Y1], method = "spearman", exact=T, continuity=T)
      cor.test(data[,X1], data[ ,Y1], method = "kendall")->Kendall 
      Resultats$"Test non paramétrique"$"Rho de Spearman"<-round(data.frame("rho"=Spear$estimate,"rho.deux"=Spear$estimate^2,"S"=Spear$statistic,"valeur.p"=Spear$p.value),4)
      round(data.frame("tau"=Kendall$estimate,"z"=Kendall$statistic,"valeur.p"=Kendall$p.value),4)->Resultats$"Test non paramétrique"$"Tau de Kendall"}
      
      
      if(!is.null(group)){
        if(choix=="Corrélations") {corr.g<-function(X2){ return(data.frame(Sp.r= cor.test(X2[, X1], X2[ ,Y1], method = "spearman")$estimate,
        ...
        if(choix!="Corrélations"){
          ...
          BPgroup<-data.frame(gr.l,BPgroup )
          Resultats$"Test non paramétrique"$"Corrélation partielle de Spearman par groupe"<-BPgroup 
        } else {dimnames(BPgroup)[[2]]<- c( "Spearman.r", "Spearman.p", "Tau.Kendall.r", "Tau.Kendall.p")
        BPgroup<-data.frame(gr.l,BPgroup )
        Resultats$"Test non paramétrique"$"Corrélation de Spearman/Kendall par groupe"<-BPgroup}
      }
    }
    
    if(any(param=="robustes"| any(param=="Test robustes - impliquant des bootstraps"))){
      boot_BP_results<-boot(data, boot_BP, n.boot)
      if(!is.null(Resultats$"Test paramétrique"$"Corrélation de Bravais Pearson")) {
        try(Resultats$"Test paramétrique"$"Corrélation de Bravais Pearson"$"Bca lim inf"<-round( boot.ci(boot_BP_results)$bca[,4],4), silent=T)
        try(Resultats$"Test paramétrique"$"Corrélation de Bravais Pearson"$"Bca lim sup"<-round( boot.ci(boot_BP_results)$bca[,5],4),silent=T)
      } else if(!is.null(Resultats$"Test paramétrique"$"Corrélation partielle/semi-partielle de Bravais Pearson")) {
        boot_BPSP_results<-boot(data, boot_BPSP, n.boot)  
        try(Resultats$"Test paramétrique"$"Corrélation partielle/semi-partielle de Bravais Pearson"$"Bca lim inf"<-round( c(boot.ci(boot_BP_results)$bca[,4], boot.ci(boot_BPSP_results)$bca[,4]),4),silent=T)
        try(Resultats$"Test paramétrique"$"Corrélation partielle/semi-partielle de Bravais Pearson"$"Bca lim sup"<-round( c(boot.ci(boot_BP_results)$bca[,5], boot.ci(boot_BPSP_results)$bca[,5]) ,4), silent=T)
      } else try(Resultats$"Analyses robustes"$"Bootstrap sur la corrélation de Bravais Pearson"<-round(data.frame("Bca.lim.inf"= boot.ci(boot_BP_results)$bca[,4], " Bca.lim.sup"=boot.ci(boot_BP_results)$bca[,5] ), 4),silent=T)
      
      if(any(param=="non param")| any(param=="Test non paramétrique")) {
        boot_Spearman_results<-boot(data, boot_Spearman, n.boot)
        if(!is.null(Resultats$"Test non paramétrique"$"Rho de Spearman")) {
          try(Resultats$"Test non paramétrique"$"Rho de Spearman"$"Bca lim inf"<-round( boot.ci(boot_Spearman_results)$bca[,4],4), silent=T)
          try(Resultats$"Test non paramétrique"$"Rho de Spearman"$"Bca lim sup"<-round( boot.ci(boot_Spearman_results)$bca[,5],4), silent=T)
        } else{
          boot_SpearmanSP_results<-boot(data, boot_SpearmanSP, n.boot)
          
          try(Resultats$"Test non paramétrique"$"Rho partiel/semi partiel de Spearman"$"Bca lim inf"<-round(c( boot.ci(boot_Spearman_results)$bca[,4], boot.ci(boot_SpearmanSP_results)$bca[,4]),4), silent=T)
          try(Resultats$"Test non paramétrique"$"Rho partiel/semi partiel de Spearman"$"Bca lim sup"<-round(c( boot.ci(boot_Spearman_results)$bca[,5], boot.ci(boot_SpearmanSP_results)$bca[,5]),4), silent=T)
        } 
    ...
    if(any(param=="Bayes") | any(param=="Facteurs bayesiens") ){
      ...
      r2Val<-r2Val^2
      BF$r.carre<-r2Val
      Resultats$"Facteurs bayesiens"$"Facteurs Bayesiens pour la corrélation de Bravais-Pearson"<-BF
      ...
      if(any(param=="non param")| any(param=="Test non paramétrique")) {
        ...
        Resultats$"Facteurs bayesiens"$"Facteurs Bayesiens pour la corrélation de Spearman"<-BFS
      ...
      Resultats$"Facteurs bayesiens séquentiels"<-.plotSBF(SBF)
