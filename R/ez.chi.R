#' Perform a contingency table analysis with optional dialog interactions
#'
#' This function performs a contingency table analysis (chi-square tests,
#' McNemar test, independence test, or Bayesian contingency test) using the
#' variables \code{X}, \code{Y}, and optionally a weighting variable.
#' Dialog interactions may be used internally via \pkg{svDialogs}.
#'
#' @details
#' The function allows:
#' \itemize{
#'   \item classical chi-square tests,
#'   \item independence tests,
#'   \item McNemar test,
#'   \item Bayesian contingency table analysis via \code{BayesFactor}.
#' }
#'
...
#' \itemize{
#'   \item \code{.dico[["txt_chi_adjustement"]]}
#'   \item \code{.dico[["txt_chi_independance"]]}
#'   \item \code{.dico[["txt_mcnemar_test"]]}
#' }
#'
...
#'   \code{data}. Must contain strictly positive integers. Optional.
#'
#' @param p A numeric vector of probabilities whose sum must be equal to 1.
...
#'   arguments.
#'
#' @param B Integer. Number of bootstrap replicates used in Monte–Carlo
#'   estimation of p-values (default: usually set inside the function).
#'
#' @param priorConcentration Numeric scalar. Prior concentration parameter
...
#'   \code{"rows"} or \code{"cols"} specifying which margin is fixed.
#'
#' @param rscale Numeric or character. Prior scale for Bayesian tests.
#'   Several preset values are available as character strings (see
#'   \code{contingencyTableBF()}).
#'
#' @return A list or object containing the results of the selected contingency
#'   table analysis. Returned structure depends on the test performed.
#'
#' @examples
#' \dontrun{
#' data <- data.frame(
#'   X = factor(sample(c("A","B"), 100, TRUE)),
#'   Y = factor(sample(c("Yes","No"), 100, TRUE)),
#'   w = sample(1:3, 100, TRUE)
#' )
#'
#' myfun(
#'   X = "X", Y = "Y",
#'   Effectifs = "w",
#'   choix = .dico[["txt_chi_independance"]],
#'   data = data
#' )
#' }
#'
#' @importFrom svDialogs dlg_message dlg_list dlg_input dlgForm
#'
#' @export
ez.chi <-
  ...
  if((choix!=.dico[["txt_chi_adjustement"]])){
    ...
    if(choix==.dico[["txt_chi_independance"]]){
      ...
      if(all(dim(tab)>2) |any(mon.chi$observed==0)) {
        Resultats[[.dico[["txt_odd_ratio"]]]]<-.dico[["desc_odd_ratio_cannot_be_computed"]]
      }else{
        if(length(tab[1,])>2) tab<-apply(tab,1, rev)
        Resultats[[.dico[["txt_odd_ratio"]]]]<- epitools::oddsratio.wald(x=tab,conf.level = 0.95,rev = c("neither"),correction = FALSE,verbose = FALSE)$measure
      }
      if(any(choix2 %in% c(.dico[["txt_non_parametric_test"]],.dico[["txt_robusts_tests_with_bootstraps"]])))      {
